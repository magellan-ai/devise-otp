  ...
  module OtpAuthenticatable
    ...
    module ClassMethods
      ::Devise::Models.config(self,
                              ...
                              :otp_drift_window,
                              :otp_trust_persistence,
                              :otp_mandatory,
                              :otp_credentials_refresh,
                              :otp_issuer,
                              :otp_recovery_tokens,
                              :otp_controller_path)

      def find_valid_otp_challenge(challenge)
        with_valid_otp_challenge(Time.zone.now).where(otp_session_challenge: challenge).first
      end
    end
    ...
    def enable_otp!
      ...
      update!(otp_enabled: true, otp_enabled_on: Time.zone.now)
    end
    ...
    def otp_challenge_valid?
      (otp_challenge_expires.nil? || otp_challenge_expires > Time.zone.now)
    end
    ...
    def next_otp_recovery_tokens(number = self.class.otp_recovery_tokens)
      (otp_recovery_counter..otp_recovery_counter + number).index_with do |index|
        recovery_otp.at(index)
      end
    end
    ...
    def validate_otp_token_with_drift(token)
      ...
      (-self.class.otp_drift_window..self.class.otp_drift_window).any? do |drift|
        time_based_otp.verify(token, at: Time.zone.now.ago(30 * drift))
      end
    end
